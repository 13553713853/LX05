  ...
  public  class Activity
    {
        ...
        public Model.Activity GetModel(int id)
        {
            ...
            {
                string sql = "select activityId,activityName,endTime," +
                    "activityPicture,summary,activityVerify,userName,recommend," +
                    "recommendtime,activityIntroduction,endTime>=now() as activityStatus" +
                    " from activity where activityId=@id";
    ...
    public IEnumerable<Model.Activity> GetVerifyPage(Model.Page page)
    {
        using (IDbConnection cn =new MySqlConnection(cns))
        {
            string sql = "with a as(select row_number() over(order by endTime desc) as num, " +
                "activityId,activityName,endTime,activityPicture,summary,activityVerify," +
                "userName,recommend,recommendtime,endTime>=now() as activityStatus from activity " +
                "where activityVerify='审核通过')";
    ...
    public IEnumerable<Model.Activity> GetRecommend()
    {
        using (IDbConnection cn =new MySqlConnection(cns))
        {
   ...
   public  Model.Activity GetEnd()
    ...
    public IEnumerable<Model.ActivityNo> GetPage(Model.Page page)
    {
        using (IDbConnection cn =new MySqlConnection(cns))
        {
            ...
            sql += "select * from a where num beteen (@pageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
            return cn.Query<Model.ActivityNo>(sql, page);
        }
    }
    public int Add(Model.Activity activity)
    {
        using (IDbConnection cn =new MySqlConnection(cns))
        {
            string sql = "insert into activity(activityname,endtime,activitypicture,activityintroduction," +
                "summary,activityverify,activitystatus,username,recommend) values(@activityName, @endTime," +
                " @activityPicture, @activityIntroduction, @summary, @activityVerify, @activityStatus, @userName, " +
                "@recommend);";
            sql += "SELECT @IDENTITY";
            return cn.ExecuteScalar<int>(sql, activity);
        }
    }
    public int Update(Model.Activity active)
    {
        using (IDbConnection cn = new MySqlConnection(cns))
        {
            string sql = "update activity set activityname=@activityName, endtime=@endTime," +
                " activitypicture=@activityPicture, activityintroduction=@activityIntroduction, " +
                "summary=@summary, activityverify=@activityVerify, activitystatus=@activityStatus " +
    ...
    public int Delete(int id)
    {
        using (IDbConnection cn =new MySqlConnection(cns))
        {
            string sql = "delect from activity where activityid=@id";
            return cn.Execute(sql, new object { id = id });
        }
    }
